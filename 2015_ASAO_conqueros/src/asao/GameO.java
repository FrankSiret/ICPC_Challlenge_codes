...
public class GameO implements Jugador {
            ...
            if(ejB1.getPlayer() == ej.getPersonalID()){
                if(ejB1.getPopulation() >= 20 && ejB1.getLevel() >= 2) {
                    for (StatusBuilding ejB2 : ejB) {
                        if(bd < bd1 && ejB2.getPlayer() != ej.getPersonalID() && ejB2.despuesEnvioEjercito().getDefense() < ejB1.getAtack() && ejB2.despuesEnvioEjercito().getPopulation() < ejB1.getPopulation()) {
                            if (men.DistanciaA(ejB1.getPosition()) > ejB2.getPosition().DistanciaA(ejB1.getPosition()))
                                men = ejB2.getPosition();
                        }    
                        else if(bd > bd1 && ejB2.getPopulation() < 40) 
                            if (men.DistanciaA(ejB1.getPosition()) > ejB2.getPosition().DistanciaA(ejB1.getPosition()))
                                men = ejB2.getPosition();
                    }
